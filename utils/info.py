...
def extract_middle_segment(env_name):
    """
    Extracts the relevant segment based on specific patterns in the input string.
    Handles cases for 'drive' and 'Offline' prefixes.
    """
    # Case 1: If 'drive' exists in the input string
    if 'drive' in env_name:
        # Split the string by hyphen '-'
        parts = env_name.split('-')
        # Ensure there are at least 3 parts to extract the middle one
        if len(parts) >= 3:
            return parts[1]  # Return the middle part
        else:
            raise ValueError("Invalid format for 'drive' environment name.")

    # Case 2: If the string starts with 'Offline'
    elif env_name.startswith('Offline'):
        # Remove 'Offline' and split by hyphen '-'
        parts = env_name[7:].split('-')  # Remove 'Offline' (7 characters)
        parts[0] = parts[0].replace('Gymnasium', '')
        return parts[0].strip()  # Return the first part without whitespace
    # Case 3: PointRobot
    elif env_name.startswith('PointRobot'):
        return 'PointRobot'

    # Case 3: If no matching pattern is found
    else:
        raise ValueError("Input does not match expected patterns ('drive' or 'Offline').")


def return_reference_scores(env_name):
    name  = extract_middle_segment(env_name)
    EPISODE_LENGTH = DEFAULT_MAX_EPISODE_STEPS[name]
    MAX_REWARD = MAX_EPISODE_REWARD[name]
    MIN_REWARD = MIN_EPISODE_REWARD[name]
    MAX_COST = MAX_EPISODE_COST[name]
    MIN_COST = MIN_EPISODE_COST[name]
    return {'max_reward':MAX_REWARD, 'min_reward':MIN_REWARD,'max_cost':MAX_COST, 'min_cost':MIN_COST,'episode_length':EPISODE_LENGTH}
